...
test("prepare local repositories without changing project configuration", () => {
	...
	try {
		...
		prepareElmExamples(root, binary, "0.4.0");
...
test("reject unsupported releases and empty executables", () => {
	...
	try {
		...
		expect(() => prepareElmExamples(root, binary, "0.2.0")).toThrow("require morphir-elm extension 0.4.0");
		expect(() => prepareElmExamples(root, binary, "0.4.0")).toThrow("executable is empty");
	} finally {
		rmSync(root, { recursive: true, force: true });
	}
});
