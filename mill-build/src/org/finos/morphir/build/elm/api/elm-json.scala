...
object ElmProject {
  ...
  final case class ElmPackage(
    ...
    @key("test-dependencies") testDependencies: Map[String, String]
  ) extends ElmProject {
...
final case class ElmPackageName(value: String) extends AnyVal {
  def toPath(root: os.Path): os.Path =
    root / os.RelPath(value.replace(".", FileSystems.getDefault().getSeparator()) + ".elm")
}
