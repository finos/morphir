...
- The workbench reuses guests through a `morphir-host` `Pool` instead of the daemon session actor. A guest is now replaced when an installed extension is reinstalled at the same version, and a result that fails to decode is retried once on a fresh guest (#963, #959).
