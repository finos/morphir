...
- The published Elm extension pin moves to `extension/elm/v0.4.0`. That release answers `morphir.extension.describe`, so `extension install` probes it by `describe`, and it also ships a version-2 process bundle: raw executables, their `.sha256` files and `morphir-elm-extension-0.4.0.bundle.release.json`, which `extension repository publish --bundle` accepts once renamed to `release.json` (finos/morphir-elm#1305, #921). The host compatibility suite expects the `describe` probe for Elm and the session fallback for `morphir-scala-elm`.
