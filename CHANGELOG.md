...
### Added
- `spec/mep/`: the Morphir Extension Protocol contract as a Gleam model, compiled to committed Morphir IR (`spec/mep/generated/mep.ir.json`) with a drift check in CI (`mise run spec:mep`, `mise run spec:mep-check`). It covers the handshake, capabilities and claims, compile, diagnostics and generate messages, and documents the pre-1.0 versioning rule and the wire mapping (#964, #960).
...
### Fixed
- `extension install` no longer refuses an extension that answers `morphir.extension.describe` when it is installed from a version-1 index record. The host converts a version-1 record into claims with its own defaults for members the flat format cannot express, such as `incremental`, and compared the `describe` answer with them exactly. It now checks that answer the way it checks a session: identity, protocol, capability kinds and the flat members. Claims an extension supplied are still checked exactly. This affected the Elm extension `v0.4.0` through its version-1 descriptor (finos/morphir-rust#261, #921).

### Changed
- **Breaking** for tooling that asserted `3.1.0` is refused: the CLI and the kit now read IR format `3.1.0`. The reference support table and the morphir-rust table move from `[3.0.0,3.1.0),[4.0.0,4.1.0)` to `[3.0.0,3.2.0),[4.0.0,4.1.0)`, so `"3.1.0"` is accepted and `3.2.0` is refused with `unsupported_format_version_minor` (kb `morphir-ir` decision 0018, [#970](https://github.com/finos/morphir/issues/970)). The reference implementation is morphir-rust finos/morphir-rust#255, #256, #257, #258 and #260, already in the pinned submodule.
- The published Elm extension pin moves to `extension/elm/v0.4.0`. That release answers `morphir.extension.describe`, so `extension install` probes it by `describe`, and it also ships a version-2 process bundle: raw executables, their `.sha256` files and `morphir-elm-extension-0.4.0.bundle.release.json`, which `extension repository publish --bundle` accepts once renamed to `release.json` (finos/morphir-elm#1305, #921). The host compatibility suite expects the `describe` probe for Elm and the session fallback for `morphir-scala-elm`. The reference Elm examples (`mise run examples:prepare-elm`) now require Elm extension `0.4.0`.
- Capability statements are now **capability claim sets** made of **claims** (kb `morphir-extensions` decision 0007, #921). The version-2 formats move to the next draft: a claim set carries `claimsVersion` `0.1.0-draft.2`, and a bundle descriptor artifact, index record or installed catalog entry holds it in `claims`, with `claimCheck` (`probed` or `unchecked`) in place of `statementSource` (`probed` or `declared`) and `schemaVersion` `2.0.0-draft.2`. This CLI still reads draft.1 records and `describe` responses and converts them, and writes only draft.2, so a Morphir home or index that this CLI wrote version-2 records into is not readable by `0.4.0-beta.6` and earlier. An installed catalog that holds any entry with claims is written as `2.0.0-draft.2`, which those CLIs cannot read at all, version-1 entries included; a catalog of version-1 entries only stays `1.0`. `extension install` and `extension list` print `Claims: unchecked` or `Claims: probed (...)`, and `--no-probe` says the claims were not checked. The `morphir-rust` pin moves to `e479dc9` (finos/morphir-rust#253).

### Added
- IR format `3.1.0`, a minor revision of the classic IR ([#970](https://github.com/finos/morphir/issues/970), kb `morphir-ir` decision 0018). A v3 distribution may be a `Specs` distribution, which publishes a package's specification without its definitions; its own modules must be module specifications, and a `Specs` that declares a release below `3.1.0` is refused with `specs_before_3_1` in JSON, YAML and Ion. A v3 distribution may also be stored as a JSON or YAML document tree, which uses the v4 tree layout with classic v3 payloads and says `"3.1.0"` in every file. Writers emit the lowest version that fits, so a single-file v3 `Library` still says `3`. `morphir migrate --target-version v3 --output-layout vfs --output-format json`, `yaml` or `ion` writes a v3 tree, and every tree format's manifest decides the version on read. A migrated v3 `Specs` becomes a v4 `Specs` with format version `4`. The v3 JSON Schema accepts `Specs`, and `docs/spec/ir/schemas/v3/document-tree-files.md` specifies the v3 tree.
- The draft Ion IR codec covers v3 dependency specifications and every v4 distribution kind and node the draft spells: specs and application distributions with entry points, every value expression and pattern, custom and derived types, and external and incomplete bodies (finos/morphir-rust#248). It also covers v4 attributes, Morphir annotations on specifications, and document literals, whose numbers keep their lexemes as an Ion int, an Ion decimal, or `number::"<lexeme>"` (finos/morphir-rust#251). `morphir ir migrate --target-version v3 --output-layout vfs --output-format ion` writes a v3 Ion tree, and `generate -i` reads one. `ionVersion` stays `0.1.0-draft.1` ([#946](https://github.com/finos/morphir/issues/946)).
