...
import Morphir.Snowpark.CommonTestUtils exposing (mMaybeTypeOf, mIdOf, mStringLiteralOf, sLit
                                                 , sCall, sVar, sDot, floatTypeInstance, sExpCall
                                                 , sIntLit, intTypeInstance, mIntLiteralOf)
...
caseOfGenTests =
    let
        ...
        (mappedCase, _) = mapValue inputCase ctx
        ...
        assertCaseWithMaybeFullCases =
            ...
            \_ ->
                let
                    ...
                    expected =
                        sCall ( applySnowparkFunc "when" [ sDot (sVar "x") "is_not_null", (sVar "x") ]
                              , "otherwise" )
            ...
            \_ ->
                let
                    ...
                    (mapped, _) =
                        mapValue inputCaseExpr ctx
                    expected =
                        sCall ( applySnowparkFunc "when" [ sDot (sVar "x") "is_not_null", applySnowparkFunc "lit" [ sLit "DEFAULT2" ] ]
                              , "otherwise" )
                ...
                let
                    ...
                    inputCaseExpr =
                        mCaseOf (mIdOf [ "x" ] timeRangeType) 
                                [ ( Value.ConstructorPattern timeRangeType (myModName "Seconds")
                                        [ Value.AsPattern intTypeInstance (Value.WildcardPattern intTypeInstance ) ["t"] ]
                                  , mIdOf [ "t" ] intTypeInstance)
                                , ( Value.ConstructorPattern timeRangeType (myModName "MinutesAndSeconds")
                    ...
                    expected =
                        sCall (
                            sCall ( applySnowparkFunc "when" [ Scala.BinOp (sExpCall (sVar "x") [ sLit "__tag" ])  "===" (applySnowparkFunc "lit" [ sLit "Seconds" ])
        ...
        assertCaseWithCustomTypeWithoutParameters =
            ...
            \_ ->
                let
                    ...
                    expected =
                        sCall (
                            sCall ( applySnowparkFunc "when" [ Scala.BinOp (sVar "x")  "===" (Scala.Ref [ "utest", "MyMod", "DeptKind" ] "Hr")
