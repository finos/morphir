...
import Morphir.Snowpark.CommonTestUtils exposing (innerJoinFunction, testDistributionPackage, testDistributionName
                                                    , mIdOf, sLit, mListTypeOf, mLambdaOf, typeA, typeB, typeC, equalFunction
                                                    , mField, intTypeInstance, mTuple2TypeOf, mRecordOf, mRecordTypeOf
                                                    , sCall, sVar, mFuncTypeOf, listMapFunction,
                                                    leftJoinFunction)
import Morphir.Snowpark.ReferenceUtils exposing (curryCall)
import Morphir.Snowpark.CommonTestUtils exposing (mMaybeTypeOf)
import Morphir.Snowpark.CommonTestUtils exposing (maybeMapFunction)
import Morphir.Snowpark.CommonTestUtils exposing (boolTypeInstance)
...
originMultipleInnerJoin =
    let
        tupleType = mTuple2TypeOf typeA typeB
    in
    curryCall (innerJoinFunction typeC tupleType,
...
originInnerJoin = curryCall (innerJoinFunction typeA typeB,
...
originLeftJoin  = curryCall (leftJoinFunction  typeA  typeB,
                                [ mIdOf ["dataSetB"] (mListTypeOf  typeB)
                                , (mLambdaOf 
                                    (["a"], typeA) 
                                    (mLambdaOf 
...
projectionInnerJoin =
    let
        tupleType = mTuple2TypeOf typeA typeB
    in
    curryCall (listMapFunction tupleType recordProjection,
        [ValueIR.Lambda 
            ...
            (ValueIR.TuplePattern 
                ...
                [ValueIR.AsPattern typeA (ValueIR.WildcardPattern typeA) ["x"]
                , ValueIR.AsPattern typeB (ValueIR.WildcardPattern typeB) ["y"]])
            (mRecordOf [("idA", (mField intTypeInstance (mIdOf ["x"] typeA) "id" )),
...
projectionLeftJoin = 
    let
        tupleType = mTuple2TypeOf typeA (mMaybeTypeOf typeB)
    in
    curryCall (listMapFunction tupleType recordMaybeProjection,
        [ValueIR.Lambda 
            ...
            (mRecordOf [("idA", (mField intTypeInstance (mIdOf ["x"] typeA) "id" )),
                        ("idB", curryCall ((maybeMapFunction typeB intTypeInstance), 
...
projectionMultipleJoin =
    let
        tupleType = mTuple2TypeOf typeA typeB
        tupleTupleType = mTuple2TypeOf tupleType typeC
    in
    curryCall (listMapFunction tupleTupleType recordMultipleProjection,
        [ValueIR.Lambda 
            ...
            (mRecordOf [("idA", (mField intTypeInstance (mIdOf ["a"] typeA) "id" ))
    ...
    let
        ...
        assertInnerJoin = 
            test ("simple inner Join") <|
                \_ ->
                    let
                        (mapped, _ ) = mapValue projectionInnerJoin ctx
                    in
        ...
        assertLeftJoin =
            test ("simple left Join") <|
                \_ ->
                    let
                        (mapped, _ ) = mapValue projectionLeftJoin ctx
                    in
        ...
        assertMultipleJoin = 
            test ("Multiple inner Join") <|
                \_ ->
                    let
                        (mapped, _ ) = mapValue projectionMultipleJoin ctx
                    in
