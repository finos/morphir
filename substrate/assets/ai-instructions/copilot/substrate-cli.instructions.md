---
applyTo: "**/*.md,**/substrate.json,**/substrate-lock.json"
...
| `substrate install` | Resolve and vendor every dependency under `substrate/`. Writes `substrate-lock.json`. Also refreshes these AI instructions. Idempotent. Pass `-f` / `--force` to reinstall everything even if already present per the lockfile. |
