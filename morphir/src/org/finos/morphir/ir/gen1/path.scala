...
object Path extends Newtype[Vector[Name]] {
...
//  @inline def toList(path: Path): List[Name] = path.toList.toList
  ...
  extension (self: Path)
    ...
    //   val nsvalue = PackageName.value(value.map(_.render))
    //   PackageName.fromIterable(nsvalue)
    // }

    // def toNamespace(implicit renderer: Name.Renderer = Name.Renderer.TitleCase): Namespace = {
    //   val nsvalue = Namespace.value(value.map(_.render))
    //   Namespace.fromIterable(nsvalue)
    // }
}
