...
The [linked-metadata draft](./linked-metadata.md) develops `attributes.facts` and scoped predicates for a future V4 minor revision. It does not change the `4.0.0` attribute encoding described here.
