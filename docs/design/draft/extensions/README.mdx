...
The extension architecture for adding capabilities to Morphir through WASM components, native executables, and the task system.

## Tracking

| Type | References |
|------|------------|
| **Beads** | morphir-go-772 (task execution), morphir-010 (CLI extensions), morphir-ct7h (distribution design), morphir-h0pf (extension host), morphir-uhk3 (package registry) |
...
|----------|--------|-------------|
| [Morphir Extension Protocol](./protocol.md) | Proposed | JSON-RPC lifecycle, capability contract, and Elm frontend delivery plan |
| [Distribution and acquisition](./distribution-and-acquisition.md) | Draft | Package registry, extension acquisition, installed catalog, and runtime selection boundaries |
| [WASM Components](../../../spec/webassembly/wasm-components.md) | Draft | Component model integration and WIT interfaces |
