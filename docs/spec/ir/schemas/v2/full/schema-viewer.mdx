...
weight: 21
description: "Interactive JSON Schema viewer for Morphir IR format version 2"
---
...
# Morphir IR Schema Version 2 - Interactive Viewer

This page provides an interactive viewer for the Morphir IR JSON Schema version 2.
...
export const SchemaViewer = () => {
  ...
  React.useEffect(() => {
    Promise.all([
      fetch('/schemas/morphir-ir-v2.json').then(r => r.ok ? r.json() : Promise.reject(`HTTP ${r.status}`)),
      fetch('/schemas/morphir-ir-v2.yaml').then(r => r.ok ? r.text() : Promise.reject(`HTTP ${r.status}`))
    ])
...
Version 2 introduced **capitalized tags** for distribution and access control:

| Element | V1 | V2 |
|---------|----|----|
| Distribution | `"library"` | `"Library"` |
| Access Control | `"public"` | `"Public"` |
| Type Tags | `"variable"` | `"Variable"` |
