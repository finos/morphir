...
module TestAggregateAverage exposing (..)
...
import SparkTests.AggregationTests exposing (testAggregateAverage)
...
encodeResult : List { product : Product, average : Float } -> String
            ...
            { encoder =
                Encode.withFieldNames
                    (\result ->
                        ...
                        , ( "average", String.fromFloat result.average )
                        ]
                    )
            , fieldSeparator = ','
            }
