...
module TestItemIsAntique exposing (..)
...
testIsItemAntique =
    let
        ...
        _ =
            Debug.log "antiques_expected_results_is_item_antique.csv" csvResults
    in
    test "Testing is_item_antique antique shop rule" (\_ -> Expect.equal (matchingAntiques |> Result.map (\list -> List.length list)) (2400 |> Ok))
