...
module TestAggregateSum exposing (..)
...
import SparkTests.AggregationTests exposing (testAggregateSum)
...
encodeResult : List { product : Product, sum : Float } -> String
            ...
            { encoder =
                Encode.withFieldNames
                    (\result ->
                        ...
                        , ( "sum", String.fromFloat result.sum )
                        ]
                    )
            , fieldSeparator = ','
            }
