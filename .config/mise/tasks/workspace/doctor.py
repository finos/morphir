#!/usr/bin/env python3
# #MISE description="Check workspace health and diagnose issues"
# #USAGE flag "--fix" help="Fix mode: replace, tags, none, prompt" default="prompt"
"""
...
def find_repo_root() -> Path:
    """Find the repository root by looking for .git or go.work"""
    current = Path.cwd()
...
def main() -> int:
    ...
    if not (root / "go.work").exists():
        print("WARN: go.work is missing (run: mise run workspace:setup)")
        issues += 1
    ...
    if missing_tags:
        if fix_choice == "replace":
            if not go_work.exists():
                print("WARN: go.work is missing; running workspace:setup before applying fixes.")
                subprocess.run(["mise", "run", "workspace:setup"], cwd=root, check=True)
