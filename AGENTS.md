...
- **Transition state:** parent IR gates use the Rust `morphir mck` commands: `check`, `coverage`, `schema check`, `run`, `report check`, `report render`, and `kit status/vendor/update`. Reports use the consolidated `2.0.0-draft.1` contract. The TypeScript driver remains for migration parity, package gates and consumers awaiting the IR-4 release/adoption cutover in [spec/mck/migration.md](spec/mck/migration.md). Keep source-copy/vocabulary parity checks until that cutover. Do not add shared MCK features in TypeScript (break/fix only), and do not delete working tooling before its Rust replacement is verified and released. Work IR first ([#851](https://github.com/finos/morphir/issues/851)); packaging follows ([#852](https://github.com/finos/morphir/issues/852)).
