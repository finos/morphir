...
- **IR cutover:** parent and audited binding IR gates use the Rust `morphir mck` commands: `check`, `coverage`, `schema check`, `run`, `report check`, `report render`, and `kit status/vendor/update`. Reports remain `2.0.0-draft.1`. The TypeScript IR runner, kit embedding and temporary parity gates are retired; the TypeScript adapter, codec tests and vocabulary authoring remain. Preserve frozen historical evidence. The native CLI also owns draft.1 package integrity and draft.2 resolution gates under [#852](https://github.com/finos/morphir/issues/852). The TypeScript package runner is retired; independent adapters, package operations and draft.3 assurance/publisher helpers remain. See [the cutover record](spec/mck/migration.md) and [#851](https://github.com/finos/morphir/issues/851).
