module Morphir.Snowpark.GenerationReport exposing ( createGenerationReport
                                                  , GenerationIssues
                                                  , GenerationIssue
                                                  , addGenerationIssue )
...
type alias GenerationIssue = String
...
generateIssuesReport issues =
    if Dict.isEmpty issues then
        ...
        ( Header2 "Generation issues" ) :: (
                issues 
                    |> Dict.toList
                    |> List.concatMap (\(func,innerIssues) -> [(Header3 (FQName.toString func)), BulletList (innerIssues |> List.map (\i -> [Span i]))]
        ))
...
generateFunctionClassificationListing (_, functionClassification, _) =
    functionClassification
        |> Dict.toList
        |> List.partition (\(_, cls) -> case cls of
